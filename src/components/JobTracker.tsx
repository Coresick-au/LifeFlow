...
import { Briefcase, Calendar, MapPin, TrendingUp, Plus, Award, Target, Info, Edit2, DollarSign } from 'lucide-react';
...
export const JobTracker: React.FC = () => {
  ...
  const [viewMode, setViewMode] = useState<'timeline' | 'income'>('timeline');
  ...
  const careerStats = useMemo(() => {
    // Only count POSITION events that are NOT synthesized "Left" events
    // These are the actual job entries with start dates (and optional end dates)
    const positionEvents = careerEvents.filter(e =>
      e.type === 'position' &&
      !e.id.endsWith('-ended') // Exclude synthesized "Left" events
    );

    const totalExperience = positionEvents.reduce((total, event) => {
      if (event.endDate) {
        return total + differenceInMonths(event.endDate, event.date);
      }
      // Only add "to now" for positions that don't have an end date (current jobs)
  ...
  return (
    <div className="bg-theme-primary rounded-lg shadow-lg p-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 border-b border-theme-border pb-4">
        <h2 className="text-2xl font-bold text-theme-primary">Career Tracker</h2>

        <div className="flex items-center gap-4">
          <div className="flex bg-theme-bg-secondary rounded-lg p-1">
            <button
              onClick={() => setViewMode('timeline')}
              className={`px-3 py-1.5 text-sm font-medium rounded-md transition-colors ${viewMode === 'timeline'
            ...
            <button
              ...
              className={`px-3 py-1.5 text-sm font-medium rounded-md transition-colors ${viewMode === 'income'
                ? 'bg-white dark:bg-slate-700 text-primary-600 dark:text-primary-400 shadow-sm'
                : 'text-theme-secondary hover:text-theme-primary'
                }`}
            >
